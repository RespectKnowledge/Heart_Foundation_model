...
1. **SSL Pretraining** — a DINOv3-style self-distillation framework built on a UxLSTM backbone, pretrained on large-scale brain MRI (UK Biobank).
2. **Downstream Adaptation** — the pretrained encoder is fine-tuned (or probed) on cardiac MRI datasets for:
...
Framework: [`nnssl`](https://github.com/RespectKnowledge/nnssl) (forked from [MIC-DKFZ/nnssl](https://github.com/MIC-DKFZ/nnssl)), included here as a git submodule.
...
|---|---|---|
| UK Biobank Brain MRI (Dataset701) | SSL pretraining | Brain MRI |
| ACDC (Dataset318) | Downstream segmentation / classification / regression | Cardiac cine MRI |
| M&Ms, M&Ms2 (Dataset521) | Downstream segmentation | Cardiac cine MRI |
| CMRxRecon2025 | Multi-label diagnosis benchmark | Cardiac cine MRI |
...
```bibtex
@inproceedings{heart_foundation_model_2026,
