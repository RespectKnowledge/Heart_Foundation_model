...
# ── Config ────────────────────────────────────────────────
INSPECT_JSON  = Path("/mnt/all_data/ssl_foundation/data/nnssl_raw/Dataset902_Inspect/pretrain_data.json")
...
OUTPUT_DIR    = Path("/mnt/all_data/ssl_foundation/data/nnssl_raw/Dataset906_All_CT")
DATASET_INDEX = 906
DATASET_NAME  = "Dataset906_All_CT"
SPLIT_RATIO   = 0.8
PROGRESS_EVERY = 5000
# ──────────────────────────────────────────────────────────
...
def add_symlink(src_path: Path, images_dir: Path):
    """Create symlink, skip if name collision."""
...
def add_flat_dataset(folder: Path, prefix: str, modality: str = "CT_noncontrast"):
    """Add flat dataset (Merlin, FLARE) — all .nii.gz in one folder."""
...
def add_nested_dataset(folder: Path, prefix: str, modality: str = "CT_noncontrast"):
    """Add nested dataset (CT-RATE) — subject/session/file.nii.gz structure."""
    global total_count, skipped_duplicates
...
# ── 1. Inspect from Dataset902 JSON ───────────────────────
print("Loading Inspect from Dataset902 JSON...")
with open(INSPECT_JSON) as f:
    inspect_data = json.load(f)

inspect_subjects = inspect_data["datasets"]["902"]["subjects"]
inspect_count = 0
for subj_id, subj in inspect_subjects.items():
    new_subj_id = f"inspect_{subj_id}"
    subjects[new_subj_id] = {"sessions": {}}
    for sess_id, sess in subj["sessions"].items():
        subjects[new_subj_id]["sessions"][sess_id] = {"images": []}
        for img in sess["images"]:
            img_path = Path(img["image_path"])
            ok = add_symlink(img_path, images_dir)
            if not ok:
                skipped_duplicates += 1
            subjects[new_subj_id]["sessions"][sess_id]["images"].append({
                "name": img["name"],
...
print(f"✅ Inspect:       {inspect_count:>8,} files")
...
print(f"\n{'='*55}")
print(f"✅ Dataset906 created successfully!")
print(f"{'='*55}")
print(f"  Inspect:          {inspect_count:>8,}")
